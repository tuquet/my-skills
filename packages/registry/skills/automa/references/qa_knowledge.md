...
| `{{$variables.<id>}}` | Tham số từ trigger | `{{$variables.url}}` |
...
```json
{
  ...
  "data": {
    ...
    "params": [
      { "name": "username", "value": "{{$variables.fb_username}}" },
      { "name": "password", "value": "{{$variables.fb_password}}" }
    ],
    "waitForResult": true,
    "assignVariable": true,
  ...
  "drawflow": {
    "nodes": [
      ...
      },
      {
        ...
        "data": { "url": "{{$variables.url}}", "active": true, "waitTabLoaded": true }
      },
      {
        "id": "n3", "label": "forms", "type": "BlockBasic",
        "position": { "x": 500, "y": 100 },
        "data": { "selector": "#email", "value": "{{$variables.email}}", "waitForSelector": true, "waitSelectorTimeout": 5000 }
      },
      {
        "id": "n4", "label": "forms", "type": "BlockBasic",
        "position": { "x": 500, "y": 250 },
        "data": { "selector": "#password", "value": "{{$variables.password}}" }
      },
      {
      ...
      },
      {
        ...
        "data": {
          "url": "{{$variables.logUrl}}",
          "method": "POST",
          "headers": [{"name": "Content-Type", "value": "application/json"}],
          "body": "{\"testCase\":\"TC-001\",\"status\":\"pass\",\"url\":\"{{$variables.url}}\"}"
        }
      },
      {
...
|------|-------------------|-------------|
| `new-tab` | `"Mở tab mới"` | `"Mở tab điều hướng đến trang login {{$variables.url}} để chuẩn bị đăng nhập bằng tài khoản {{$variables.username}}"` |
| `forms` | `"Nhập user"` | `"Điền username {{$variables.username}} vào ô #username, dùng delay 50ms để tránh chống bot"` |
...
| `loop-data` | `"Vòng lặp"` | `"Lặp {{$variables.scroll_page}} lần, mỗi lần cuộn xuống 1000px để load thêm nội dung lazy"` |
| `webhook` | `"Gửi log"` | `"Gửi log success/fail về server {{$variables.logUrl}} để team theo dõi tỷ lệ login thành công"` |
